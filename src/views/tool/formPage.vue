...
该table表示detail的所有字段
    ...
    12：暂时搜索列表只支持过去时间，dialog只支持未来时间
...
`
  ...
  .key || item.key}
    ...
    textInit() {
      ...
      this.form.hasFile = hasFile
      this.form.hasPhone = hasPhone
      this.form.hasAddress = hasAddress
      this.form.hasDialogTime = hasDialogTime
      this.form.hasSearchTime = hasSearchTime
      this.form.hasEnterprise = hasEnterprise
      this.form.hasCertNo = hasCertNo
      this.form.hasBankCardNo = hasBankCardNo
...
<style lang="scss" scoped>
   ...
   `
    ...
    submitForm() {
      this.form.keyList.forEach(item => {
        item.dialog.value = true
        item.table.value = false
        item.search.value = false
      })
      this.textInit()
      const text = `<template>
          ${this.getDialogForm()}
  ...
  this.form.hasAddress ? 'cnCity: cityJson.data,\n' : ''
    ...
    subFormMethods() {
      ...
      this.form.keyList
        ...
        .forEach(item => {
          test += `changeTimeFormDetail${item.dialog.combination.key1}(val){
          this.formDetail.${item.dialog.combination.key1 || item.key} = val[0]
          this.formDetail.${item.dialog.combination.key2 || item.key} = val[1]
          },
          `
        })
      return test
